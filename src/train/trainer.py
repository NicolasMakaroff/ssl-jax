...
class Trainer(TrainerModule):
    ...
    def create_functions(self):
        def loss_fn(params, state, loss_scale, rng, batch, is_training):
            ...
            output = self.forward.apply(params, state, rng, batch, is_training=is_training)
            if self.model_name == 'simsiam':
                metrics = self._get_simsiam_loss(output[0])
                loss = metrics["loss"]
                state = output[1]
            else:
                (loss, metrics), state = output if is_training else (output, None)
        ...
        def train_step(train_state, batch):
            ...
            self.state = TrainState(new_params, new_state, new_opt_state, loss_scale, rng=next_rng_key)
            return self.state, metrics, grads

        def eval_step(train_state, batch):
            _, (metrics, _) = loss_fn(train_state.params, train_state.state, train_state.loss_scale, train_state.rng, batch, is_training=True)
            return metrics
