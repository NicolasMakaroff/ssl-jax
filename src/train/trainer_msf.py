...
from src.dataloader.augment import parallel_augment, test_augment
import numpy as np
...
class Trainer(TrainerModule):
    ...
    def create_functions(self):
        def loss_fn(params, target_params, state, target_state, loss_scale, rng, queue, queue_ptr, batch, is_training):
            batch = parallel_augment(rng, batch)
            size = batch.shape[0]//2
            (_, query), online_state = self.forward.apply(
                params=params,
                state=state,
                rng=rng,
                batch=batch[:size],
                is_training=True)
            (current_target, _), target_state = self.forward.apply(
            ...
            current_target = l2_normalize(current_target)
            queue = jax.lax.dynamic_update_slice(queue, current_target, (ptr, ptr + size))
            ptr = (ptr + size) % self.hparams['mem_bank_size']  # move pointer
            queue_ptr = ptr
            targets = stop_gradient(queue.copy())
            dist_t = 2 - 2 * jnp.einsum('bc,kc->bk', stop_gradient(current_target), targets)
            dist_q = 2 - 2 * jnp.einsum('bc,kc->bk', query, targets)
            _, nn_index = jax.lax.approx_min_k(dist_t, k=self.hparams['top_k'])
            nn_dist_q = jax.numpy.take_along_axis(dist_q, nn_index, axis=1)
            loss = jnp.mean(jnp.sum(nn_dist_q, axis=1) / self.hparams['top_k'])
            metrics = dict(loss=loss)

            return loss, (metrics, dict(online_state=online_state, target_state=target_state), queue, queue_ptr)
        

        def train_step(train_state, batch, epoch_idx):
            params, target_params, state, target_state, opt_state, loss_scale, rng, queue, queue_ptr = train_state
            #loss_fn = lambda params: loss_function(params, state.batch_stats, state.rng, batch, is_training=True)
            
            grads, (metrics, new_state, new_queue, new_queue_ptr) = (
                jax.grad(loss_fn, has_aux=True)(params, target_params, state, target_state, loss_scale, rng, queue, queue_ptr, batch, is_training=True))
            ...
            self.state = TrainState(new_params, target_params, new_state['online_state'], new_state['target_state'], new_opt_state, loss_scale, rng=next_rng_key, queue=new_queue, queue_ptr=new_queue_ptr)
            
            
            return self.state, metrics, grads

        def eval_step(train_state, batch):
            _, (metrics, _, _, _) = loss_fn(train_state.params, train_state.target_params, train_state.state, train_state.target_state, train_state.loss_scale, train_state.rng, train_state.queue, train_state.queue_ptr, batch, is_training=False)
            return metrics
